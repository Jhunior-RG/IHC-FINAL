...
const MisPedidos = () => {
  const [pedidosEnCurso] = useState([
    ...
    },
    {
      ...
      tracking: [
        { estado: "confirmado", fecha: "2024-01-21 11:20", completado: true },
        { estado: "preparando", fecha: "2024-01-21 15:30", completado: true },
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Rastrea y gestiona todos tus pedidos</p>
      </div>
      ...
      <Tabs defaultValue="en-curso" className="space-y-6">
        <TabsList className="grid w-full grid-cols-2">
          ...
          <TabsTrigger value="pasados" className="flex items-center gap-2">
            ...
            {pedidosEnCurso.map((pedido) => (
              <Card key={pedido.id} className="overflow-hidden">
                <CardHeader className="bg-gray-50">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-bold mt-1">Bs. {pedido.total}</p>
                    </div>
                  </div>
                </CardHeader>
                ...
                <CardContent className="p-6">
                  <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium text-blue-900">Dirección de Entrega</h4>
                        <p className="text-blue-700">{pedido.direccionEntrega}</p>
                        <p className="text-sm text-blue-600 mt-1">
                          Entrega estimada: {new Date(pedido.estimadoEntrega).toLocaleDateString("es-ES")}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Tracking */}
                  <div className="mb-6">
                    <h4 className="font-medium mb-4">Estado del Pedido</h4>
                    <div className="flex items-center justify-between relative">
                      {pedido.tracking.map((step, index) => (
                        <div key={index} className="flex flex-col items-center flex-1 relative z-10">
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center ${
                              step.completado ? "bg-green-500 text-white" : "bg-gray-200 text-gray-500"
                            }`}
                          >
                            {getEstadoIcon(step.estado)}
                          </div>
                          <p
                            className={`text-xs mt-2 text-center ${step.completado ? "text-green-600" : "text-gray-500"}`}
                          >
                            {step.estado.replace("-", " ").charAt(0).toUpperCase() +
                              step.estado.replace("-", " ").slice(1)}
                          </p>
                          {step.fecha && (
                      ...
                      <div className="absolute top-4 left-0 right-0 h-0.5 bg-gray-200 -z-10">
                        <div
                          ...
                          style={{
                            width: `${(pedido.tracking.filter((s) => s.completado).length - 1) * (100 / (pedido.tracking.length - 1))}%`,
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="font-medium">Productos ({pedido.productos.length})</h4>
                    {pedido.productos.map((item, index) => (
                      <div key={index} className="flex items-center gap-4 p-3 border rounded-lg">
                        <Image
                          src={item.producto.image || "/placeholder.svg"}
                        ...
                        />
                        <div className="flex-1">
                          <h5 className="font-medium">{item.producto.name}</h5>
                          <p className="text-sm text-gray-600">Cantidad: {item.cantidad}</p>
                          <p className="text-sm text-gray-500">Bs. {item.precioUnitario} c/u</p>
                        </div>
                        <p className="font-medium">Bs. {item.cantidad * item.precioUnitario}</p>
                      </div>
                    ))}
                  </div>
                    ...
                    <div>
                      ...
                      {pedido.fechaEntrega && (
                        <p className="text-sm text-green-600 flex items-center gap-2">
                          ...
                          Entregado el {new Date(pedido.fechaEntrega).toLocaleDateString("es-ES")}
                        </p>
                      )}
                    </div>
                    <div className="text-right">
                ...
                <CardContent className="p-6">
                  ...
                  {pedido.estado === "entregado" && pedido.calificacion && (
                    <div className="mb-4 flex items-center gap-2">
                      ...
                      <div className="flex">{renderStars(pedido.calificacion)}</div>
                    </div>
                  )}
                  {pedido.estado === "cancelado" && pedido.motivoCancelacion && (
                    <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                      <p className="text-sm text-red-800">
                        <strong>Motivo de cancelación:</strong> {pedido.motivoCancelacion}
                      </p>
                    </div>
                  )}
                  <div className="space-y-3">
