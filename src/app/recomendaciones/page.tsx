...
import { Heart, ArrowLeft, ArrowRight, CheckCircle, Sparkles, PawPrint, Award, Target, Zap, Search } from "lucide-react"
...
const completarProducto = (producto: any) => {
  ...
  return {
    ...
    tamanos: ["Pequeño", "Mediano", "Grande"],
    pesos: ["< 5kg", "5-15kg", "15-30kg", "> 30kg", "< 2kg", "2-5kg", "5-10kg", "> 10kg"],
    razas: ["Labrador", "Pug", "Pastor Alemán", "Siames", "Persa", "Siberiano", "Otro"],
  }
}
...
const Page = () => {
  ...
  const [filteredProducts, setFilteredProducts] = useState<ProductRecomendado[]>([])
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">Productos Recomendados</h1>
        <p className="text-gray-600 max-w-2xl mx-auto mb-6">
          Descubre los mejores productos para tu mascota con nuestro sistema de recomendaciones inteligente
        </p>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium text-primary">{Math.round(progress)}% completado</span>
                </div>
                  ...
                  return (
                    <div key={index} className="flex items-center gap-4">
                      <div className="flex flex-col items-center text-center">
                        ...
                        <div className="mt-2">
                          <p className={`text-xs font-medium ${isActive ? "text-primary" : "text-gray-600"}`}>
                            {stepItem.title}
                          </p>
                        </div>
                      </div>
                      {index < steps.length - 1 && (
                        <div className={`w-12 h-0.5 ${step > index ? "bg-green-500" : "bg-gray-200"}`} />
                      )}
                    </div>
                  )
                })}
              ...
              <Card className="border-0 shadow-sm">
                <CardHeader className="bg-gradient-to-r from-primary/5 to-purple/5 text-center">
                  <CardTitle className="text-xl font-bold text-gray-900">{steps[step].title}</CardTitle>
                ...
                <CardContent className="p-6">
                  ...
                  >
                    {step === 0 && <Step0 selected={pet} setSelected={setPet} />}
                    {step === 1 && (
                      <Step1
                        selected={pet}
                        setSelected={setPet}
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-4">
            {filteredProducts.map((product) => (
              <CardRecomended key={product.id} product={product} filterTags={selectedTags} />
            ))}
          </div>
        </div>
      )}
      {filteredProducts.length === 0 && (
        <div className="text-center py-12 mb-30">
          <div className="max-w-md mx-auto">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">¿Listo para encontrar el alimento perfecto?</h3>
            <p className="text-gray-600 mb-6">
              Usa nuestro recomendador inteligente para descubrir los mejores productos para tu mascota
            </p>
          </div>
        </div>
      )}
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-gray-900 mb-2">¿Qué tipo de mascota tienes?</h3>
        <p className="text-gray-600 text-sm">Selecciona tu compañero para personalizar las recomendaciones</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 max-w-md mx-auto">
        <div
          ...
          className={`group cursor-pointer transition-all duration-300 ${
            selected === "perro" ? "ring-2 ring-primary bg-primary/5 scale-105" : "hover:scale-105 hover:shadow-md"
          }`}
        >
          <Card className="h-60 border-2 border-transparent group-hover:border-primary/20 transition-all duration-300">
            <CardContent className="p-4 text-center">
              <div className="relative mb-3">
                <Image src="/golden.png" alt="Perro" width={80} height={80} className="mx-auto rounded-full" />
                {selected === "perro" && (
              ...
              <h4 className="font-semibold text-gray-900 mb-1">Perro</h4>
              <p className="text-xs text-gray-600">Nutrición canina</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div
          ...
          className={`group cursor-pointer transition-all duration-300 ${
            selected === "gato" ? "ring-2 ring-primary bg-primary/5 scale-105" : "hover:scale-105 hover:shadow-md"
          }`}
        >
          <Card className="h-60 border-2 border-transparent group-hover:border-primary/20 transition-all duration-300">
            <CardContent className="p-4 text-center">
              <div className="relative mb-3">
                <Image src="/gatoJoy.png" alt="Gato" width={80} height={80} className="mx-auto rounded-full" />
                {selected === "gato" && (
                  <div className="absolute -top-1 -right-1 bg-primary text-white rounded-full p-1">
                    <CheckCircle className="h-4 w-4" />
                  </div>
              ...
              <h4 className="font-semibold text-gray-900 mb-1">Gato</h4>
              <p className="text-xs text-gray-600">Nutrición felina</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      {selected && (
        <div className="text-center animate-fade-in-up">
          <div className="inline-flex items-center gap-2 bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm">
            <CheckCircle className="h-4 w-4" />
            <span className="font-medium">¡Perfecto! Has seleccionado {selected}</span>
          </div>
        </div>
      )}
    </div>
...
const Step1 = ({
  ...
  setSelectedTags: (tags: { raza: string; edad: string; tamano: string; peso: string }) => void
  onClose: () => void
}) => {
  const razasPerros = ["Labrador", "Pug", "Pastor Alemán", "Otro"]
  const razasGatos = ["Siames", "Persa", "Siberiano", "Otro"]
  const edades = ["Cachorro (0-1 año)", "Adulto (1-7 años)", "Senior (7+ años)"]
  ...
  const handleConfirm = async () => {
    ...
    const productosConScore = productosCompletos.map((p) => {
      let score = 0
      if (selected && (p.especie === "ambos" || p.especie === selected)) score++
  ...
  return (
    <div className="space-y-4">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Cuéntanos sobre tu {selected}</h3>
        <p className="text-gray-600 text-sm">Esta información nos ayudará a encontrar la nutrición perfecta</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Select value={raza} onValueChange={setRaza}>
            ...
            <SelectContent>
              {(selected === "perro" ? razasPerros : razasGatos).map((r) => (
        ...
        <div className="space-y-2">
          ...
          <Select value={peso} onValueChange={setPeso}>
            ...
            <SelectContent>
              {(selected === "perro" ? pesosPerros : pesosGatos).map((p) => (
      ...
      {isFormComplete && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 animate-fade-in-up">
          <div className="flex items-center gap-2 text-green-800">
            <CheckCircle className="h-4 w-4" />
            <span className="font-medium text-sm">¡Información completa!</span>
          </div>
          <p className="text-green-700 text-xs mt-1">
            Ya podemos generar recomendaciones personalizadas para tu {selected}
          </p>
        </div>
      )}

      <div className="flex flex-col sm:flex-row gap-3 justify-between pt-4">
        <Button variant="outline" onClick={() => setStep(0)} className="rounded-full px-6" disabled={isLoading}>
          <ArrowLeft className="h-4 w-4 mr-2" />
          Volver
        </Button>

        <Button onClick={handleConfirm} disabled={!isFormComplete || isLoading} className="rounded-full px-6" size="lg">
          {isLoading ? (
